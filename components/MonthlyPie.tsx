...
import useFetchMonthlyPie from "@/custom_hooks/useFetchMonthlyPie";
...
export default function MonthlyPie() {
  ...
  if (isError) {
    console.log(isError);
    throw new Error("Unable to fetch data. MonthlyPie rh: api/fetchMonthlyPie");
  }
  ...
  return (
    <div className="border border-neutral pb-5">
      ...
      <form onSubmit={handleSubmitMonth} className="flex items-end px-5">
        <div className="form-control w-full max-w-xs">
          <label className="label">
            <span className="label-text font-semibold">Choose a month</span>
          </label>
          <select
            className="select select-neutral select-bordered select-xs"
            id="month"
            name="month"
            defaultValue={month_Pie}
          >
